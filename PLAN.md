...
| 12.3 | Annotation processor order conflict | BUILD_OUTPUT | `build_line_contains`: "Lombok" and "MapStruct" conflict; getter not found during MapStruct generation | "Lombok and another annotation processor (commonly MapStruct) are running in the wrong order, so Lombok-generated methods are absent when the second processor needs them." | "Declare Lombok before MapStruct in the annotationProcessor configuration in your build file; processor order matters because Lombok must generate getters before MapStruct can read them." |
...
| M6 | BUILD_OUTPUT tap | Compile-phase rules (10.5, 10.6, 12.x) passing |
| M7 | Remaining catalog sections | All 80+ rules in DONE status with passing fixtures |
