...
- **Code convention checks** — a separate, proactive analysis layer (think FindBugs/SonarLint, but with rules we define) that flags convention violations as inspection warnings in the editor, in *Analyze > Inspect Code*, and at commit time. Driven by its own catalog ([`conventions.yaml`](src/main/resources/rules/conventions.yaml)); each rule turns on and off in Settings. Covers Javadoc on public methods, Robot Framework integration test conventions (required suite metadata, Test ID format, per-test-case `[Documentation]` and requirement `[Tags]`, see [`docs/robot-test-conventions.md`](docs/robot-test-conventions.md)), and Spring Boot layering conventions (constructor injection over field injection, no `System.out`/`System.err`, `Optional` not used as a parameter or field, `@Transactional` kept off controllers/repositories, bounded `String` fields on JPA entities, `@Valid` paired with `@RequestBody`, `@Service` class naming, versioned controller base paths — see [`docs/spring-boot-conventions.md`](docs/spring-boot-conventions.md)). The Spring Boot rules default to `WEAK_WARNING` and are phrased as suggestions, not mandates, since they are team preferences rather than hard errors; each still turns off individually in Settings.
...
- **Convention checks** — Javadoc, Robot Framework, and Spring Boot layering rules shipped; remaining: confirm `.robot` highlighting live in the IDE (file-type association is verified by hand), get team sign-off on the Spring Boot rule params (annotation lists, path pattern, name suffix — all adjustable in `conventions.yaml`), mirror the convention engine to VS Code, optional `LocalQuickFix` to insert a Javadoc stub (see [CONVENTIONS-PLAN.md](CONVENTIONS-PLAN.md))
