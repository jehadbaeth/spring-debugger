...
- **Convention checks** — Javadoc and Robot Framework rules shipped; remaining: confirm `.robot` highlighting live in the IDE (file-type association is verified by hand), mirror the convention engine to VS Code, optional `LocalQuickFix` to insert a Javadoc stub (see [CONVENTIONS-PLAN.md](CONVENTIONS-PLAN.md))
